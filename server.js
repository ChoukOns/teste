...
////////////////////////// liste achat apprenant //////////////
app.get('/apprenants/:nom/cours-achetes', async (req, res) => {
  try {
    const nomApprenant = req.params.nom;

    // Trouver l'apprenant par nom
    const apprenant = await Apprenant.findOne({ nom: nomApprenant });

    if (!apprenant) {
      return res.status(404).send('Aucun apprenant trouvé avec ce nom');
    }

    // Trouver les achats de l'apprenant et peupler les détails des cours
    const achats = await Achat.find({ apprenant: apprenant._id }).populate('cours');

    if (!achats || achats.length === 0) {
      return res.status(404).send('Aucun cours acheté trouvé pour cet apprenant');
    }

    // Récupérer les détails des cours achetés
    const coursAchetes = achats.map(achat => achat.cours);
    res.json(coursAchetes);
  } catch (err) {
    console.error(err);
...
//////////////////////::::traduction////////////////////
const UPLOADS_DIR = 'C:\\Users\\MSI\\pfe\\backend_pfe\\public\\uploads';

const { execFile } = require('child_process');


// Configuration de multer pour gérer les téléchargements de fichiers
const upload1 = multer({ dest: `${UPLOADS_DIR}/original_files` });

// Endpoint pour télécharger et traduire un fichier texte
app.post('/translate-text-file', upload1.single('file'), (req, res) => {
  try {
    ...
    const inputFilePath = `C:\\Users\\MSI\\pfe\\backend_pfe\\public\\uploads\\original_files\\${file}`;
    // const outputFilePath = path.join(UPLOADS_DIR, 'translated_files', `${file.originalname.split('.')[0]}_translated.txt`);
    const outputFilePath = path.join(UPLOADS_DIR, 'translated_files', `${file.split('.')[0]}_translated.txt`);

    // Ajustez le chemin ici pour inclure le répertoire public
    execFile('python', ['public/script2.py', inputFilePath, outputFilePath, targetLanguage], (error, stdout, stderr) => {
      if (error) {
        console.error(`exec error: ${error}`);
        return res.status(500).json({ error: 'Internal server error' });
      }

      // Lire et afficher le contenu du fichier traduit
      fs.readFile(outputFilePath, 'utf-8', (err, data) => {
        if (err) {
          console.error(`Error reading file: ${err}`);
          return res.status(500).json({ error: 'Error reading translated file' });
        }

        console.log(`Translated text:\n${data}`);

        return res.status(200).json({ success: 'File uploaded and translated successfully', outputFilePath, translatedText: data });
      });
    });
  } catch (error) {
    console.error(error);
...
app.get('/api/file-content2', async (req, res) => {
  const fileName = req.query.fileName;
  const filePath = path.join(__dirname, 'public', 'uploads', 'translated_files', fileName); // Formez le chemin absolu du fichier
...
app.get('/download-file/:id', async (req, res) => {
  try {
      ...
      if (!cours || !cours.fichier) {
          return res.status(404).send('Fichier non trouvé');
      }

      // const filePath = path.join(__dirname, 'public', 'uploads', 'original_files', cours.fichier);
      const fileName = `${cours.fichier.split('.')[0]}_translated.txt`;
      const filePath = path.join(UPLOADS_DIR, 'translated_files', fileName);

      // Utiliser res.download pour déclencher le téléchargement du fichier
